...
-- clamp val 
	...
	local function setInDev(useTch)
		if useTch == true then
			oLvIn = {hit = tchHit}
		else
			oLvIn = {hit = bxHit}
	...
	function oLvgui.createTheme(fntsize)
		if fntsize == nil then
			fntsize = 16
		end
		local lvTheme = {}
		lvTheme.drop = {x = 7, y = 6}		-- dropshadow x,y
		lvTheme.font = love.graphics.newFont(fntsize)
		lvTheme.fontsize = fntsize
...
-- ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
	...
	oLvgui.drawMark = function (x, y, sx, sy, verts, mode)
		drawPoly (x - sx/2, y - sy/2, sx, sy, verts, mode)
	end
...
-- ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
	...
	local function upDPa(v, tid, tx, ty)
	...
	function updatePanel(v, dt)
		if v.visible == 1 and v.active == 1 and v.etype == 1 then
			if blockAll == 0 then		-- blockAll freezes all elements if a droplist is open
				if v.sync == 1 then
					-- a sync callback sends a 3 as state, not a 0, 1 or 2
					-- but it doesn't set the state field
					v.callback(3, v.user, (v.lasttx - v.x) / v.width, (v.lastty - v.y) / v.depth)
					v.sync = 0
				end

				local tid, tx, ty = oLvIn.hit(v.x, v.y, v.width, v.depth)
				v.tx, v.ty = tx, ty

				if vPort.useTouch == true or mouse.b >= 1 then 
					upDPa(v, tid, tx, ty)
...
-- ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
	function doBsync(v)
		v.textOrig = (v.width - v.theme.font:getWidth(v.text)) /2
	end
	...
	function oLvgui.createButton(list, text, options, x, y, width, depth, user, callbk)
		...
		lvButton.text = text
		lvButton.textOrig = 0 --(width - theme.font:getWidth(text)) /2
		lvButton.x = x
		lvButton.y = y
...
--				if IDlock == v.guiID and mouse.b >= 1 then  -- body == selcolor only when clicked
				if v.press == 1 and mouse.b >= 1 then
					love.graphics.setColor(v.theme.cmplColor)
	...
	local function upDBu(v, tid, tx, ty)
		if tid ~= -1 then   --  touch inside
			if mouse.b == 2 or vPort.useTouch == true then
				if v.press == 0 then
					if v.etype == 1 then
						if v.state == 1 then
							v.state = 0
	...
	function updateButton(v)
		if v.visible == 1 and v.active == 1 and blockAll == 0 then
			...
			if vPort.useTouch == true or mouse.b >= 1 then 
				upDBu(v, tid, tx, ty)
			elseif vPort.useTouch == false and mouse.b ==0 then  -- deselect by moving out of panel
				v.press = 0
			end
		end
	end
-- ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
	function oLvgui.createSlider(list, label, options, x, y, width, depth, value, min, max, user, callbk)
		...
		for i,_ in ipairs(options) do
			...
			elseif options[i] == 'SHOWV_ON' then
				lvSlider.showValue = true
			elseif options[i] == 'SHOWV_OFF' then
				lvSlider.showValue = false
			elseif options[i] == 'RETURN_ON' then
				lvSlider.autoRet = true
			elseif options[i] == 'RETURN_OFF' then
				lvSlider.autoRet = false
			end
		end
		...
		if v.visible == 1 then
			if v.active == 1 then
				...
				elseif v.etype == 'H' then
					...
					gprint(v.label, v.x + 12, v.y - v.theme.fontsize - 4)
				end
				love.graphics.pop()
			else
				love.graphics.setColor(v.theme.shadowColor) -- inactive
	...
	local function upDSl(v, tid, tx, ty)
	...
	function drawTxbox(v)
		...
		if v.visible == 1 then
			if v.active == 1 then
				...
				love.graphics.setColor(v.theme.labelColor)
				gprint(v.label, v.x + 12, v.y - v.ftHt -2)
			else
				love.graphics.setColor(v.theme.shadowColor) -- inactive
				love.graphics.rectangle("line", v.x, v.y, v.width, v.depth, 4, 4)
			end
...
-- ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
	function getFirstUni(t)
		return utf8.char(utf8.codepoint(t, 1, 1))
	end

	function stripFirstUni(s)
			...
			if v.active == 1 then
				...
				if v.focusType ~= 3 then -- ------------------------------  not menu
					...
					for i=1, v.itemsTotal, 1 do
						...
						if i == v.selected then
							...
							gprint(string.sub(v.items[i], 1, lleng), v.x + 12, v.y + 6 - v.depth / 12 )
						end
					end
					if v.selected == 0 then
						love.graphics.setColor(v.theme.labelColor)
						gprint('(none)', v.x + 12, v.y + 6 - v.depth / 12 )			
					end
				else
					-- ------------------------------------------------  menu
					love.graphics.setColor(v.theme.outline)
				...
				if IDlock == v.guiID then
					...
					for i=1, v.itemsShow, 1 do
						...
						if getFirstUni(v.items[i + v.itemsTop]) == MENU_SEL_CHAR then
							love.graphics.setColor(v.theme.dlFontColor)
							local ustr = stripFirstUni(v.items[i + v.itemsTop])
							gprint(string.sub(ustr, 1, lleng - 1), v.offsetx + v.x + 10, v.offsety +  v.y + v.depth * (i - 1) + 1)
						else
	...
	function drawKnob(v)
		...
		if v.visible == 1 then
			if v.active == 1 then
				...
				-- label
				love.graphics.setColor(v.theme.labelColor)
				gprint(v.label, v.x  - v.labelLen/2, v.y - v.radius - v.theme.fontsize - 4)
				-- knob
				love.graphics.setColor(v.theme.shadowColor)
...
-- access functions
	...
	function oLvgui.setLabelByElement(gList, elem, label)
		if elem ~= nil then				
			if elem.kind == 'sl' then
				if elem.txtype == 'V' then
					gList[index].labelsplit = splitStr(label)
				end
			elseif elem.kind == 'bu' then
				elem.text = label
			elseif elem.label ~= nil then
				elem.label = label
			end
			elem.sync = 1
		end
	...
	function oLvgui.setLabelByIndex(gList, index, label)
		index = math.floor(index)
	...
	function oLvgui.dlMenuSelect(elem, item)
		if elem ~= nil then
			if elem.kind == 'dl' then
				if item > 0 and item < elem.itemsTotal then
					if getFirstUni(elem.items[item]) ~= MENU_SEL_CHAR then
						elem.items[item] = MENU_SEL_CHAR..elem.items[item]
					end
				end
			end
		end
	...
	function oLvgui.dlMenuUnSelect(elem, item)
		if elem ~= nil then
			if elem.kind == 'dl' then
				if item > 0 and item < elem.itemsTotal then
					if getFirstUni(elem.items[item]) == MENU_SEL_CHAR then
						elem.items[item] = stripFirstUni(elem.items[item])
					end
				end
			end
		end
	...
	function oLvgui.dlMenuIsSelected(elem, item)
		local rv = false
		if elem ~= nil then
			if elem.kind == 'dl' then
				if item > 0 and item < elem.itemsTotal then
